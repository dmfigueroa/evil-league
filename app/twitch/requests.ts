...
export async function getStreams({
  ...
  const params = new URLSearchParams();
  for (const participant of channels) {
    params.append("user_login", participant);
  }

  return (
    await (
      await fetch(`https://api.twitch.tv/helix/streams?${params.toString()}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Client-Id": clientId,
        },
      ...
      data: { user_id: string; user_name: string; viewer_count: number }[];
    }>()
  ).data;
}
